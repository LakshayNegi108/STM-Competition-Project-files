...
void LCD_DrawVBorder(int16_t x, int16_t y, int16_t h, uint16_t color,
		int16_t BorderSize) {
...
void LCD_DataDisplayBox(uint16_t x, uint16_t y, int16_t w, int16_t h,
		uint16_t borderColor, uint16_t textColor, uint16_t textBgColor, float Val,
		char *title) {

	LCD_DrawRect(x, y, w, h, borderColor);
	LCD_SetCursor((x + w) / 2, y + 5);
	LCD_SetTextColor(textColor, textBgColor);
	LCD_SetTextSize(2);
	LCD_Printf(title);

	LCD_SetTextSize(3);
	LCD_SetTextColor(textColor, textBgColor);
	LCD_SetCursor((x + w)/2, (y + h)/2);
	LCD_Printf("%0.2f", Val);
}
